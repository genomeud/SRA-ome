...
BASEDIR=/mnt/vol1/projects/SRAome
DBDIR=$BASEDIR
READDIR=${BASEDIR}/reads/Metatranscriptomics
KRAKBIN=${BASEDIR}/kraken2/
KDB=${BASEDIR}/database
KRES=${BASEDIR}/classification/Metatranscriptomics
LOGDIR=${BASEDIR}/logs

module load it/lang/r/3.6.1
#Needed only once
# Rscript ${BASEDIR}/functions/01_explore_SRA.r \
# -D FALSE \
# -S SRAmetadb.sqlite \
# -F $DBDIR \
...
#From newer to older
Rscript ${BASEDIR}/02_download_metrans_SRA.r \
-S ${DBDIR}/SRAmetadb.sqlite \
-F $READDIR \
-R TRUE \
-D 30 \
-I ${DBDIR}/metatranscriptomics_table.txt 

myd_for=`echo "module load it/lang/r/3.6.1; Rscript /projects/populus/ep/share/marroni/functions/EOSC/02_download_metrans_SRA.r \
-S ${DBDIR}/SRAmetadb.sqlite \
-F $READDIR \
-R FALSE \
-D 30 \
...
#To run in screen we need this compiler, I don't know why!!
compilers/gcc/5.4.0_novabreed
NPROCS=8
${BASEDIR}/scripts/02_class_metag.sh $KRAKBIN $KDB $KRES $READDIR $LOGDIR $NPROCS



#mykmini=`qsub -F "$MINIKDB $MINIKRES $READDIR" /projects/populus/ep/share/marroni/scripts/EOSC/01_class_metag.pbs -W depend=afterany:$myd_for:$myd_rev` 
...
qsub -F "$MINIKRES SRR 4" /projects/populus/ep/share/marroni/scripts/EOSC/03_move_kres.pbs -W depend=afterok:$mykmini 
qsub -F "$MINIKRES ERR 4" /projects/populus/ep/share/marroni/scripts/EOSC/03_move_kres.pbs -W depend=afterok:$mykmini 
qsub -F "$MINIKRES DRR 4" /projects/populus/ep/share/marroni/scripts/EOSC/03_move_kres.pbs -W depend=afterok:$mykmini 




#To check the progress quickly!
#cat d_rev.err| grep -v "\\.\\.\\.\\." | less
#Check how many donwload were completed or attempted
cut -f43  ${DBDIR}/metatranscriptomics_table.txt | sort | uniq -c
