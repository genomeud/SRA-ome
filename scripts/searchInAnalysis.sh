...
then
    echo -e "usage: $0 <folderSearchFiles> <field> <stringToSearch> [<folderOutput>] \n"
...
field=$2
stringToSearch=$3
folderOutput=$folderSearch

if test $# -gt $nOfParamsNeeded
then
    folderOutput=$4
    mkdir $folderOutput 2>/dev/null
fi
...
outputFile=$folderOutput/'searched_'$stringToSearch'_in_field_'$field'.txt'
