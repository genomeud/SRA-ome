...
option_list = list(
  ...
  make_option(c("-I", "--infile"), type="character", default="/projects/populus/ep/share/marroni/EOSC/metagenomics_table.txt",
  ...
  make_option(c("-F", "--destFolder"), type="character", default="/projects/populus/ep/share/marroni/EOSC/reads/Metagenomics",
...
onlyq<-fread(infile,data.table=F,quote="")
...
downloadMe<-ifelse(reverse,max(which(is.na(onlyq$downloaded))),min(which(is.na(onlyq$downloaded))))
#Get unique names of runs corresponding to the SRA accession
getMe<-unique(sraConvert(onlyq$submission_accession[downloadMe],sra_con=con)$run)
...
dres<-try(getSRAfile( getMe, con, destDir=destFolder, fileType = 'fastq' ))
